        ...
        mp_srcptr vp, mp_size_t vn)
{
...
#if WANT_FFT
  if (ABOVE_THRESHOLD(un + vn, 2 * MUL_FFT_FULL_THRESHOLD) && ABOVE_THRESHOLD(3 * vn, MUL_FFT_FULL_THRESHOLD))
  {
  ...
  if (ABOVE_THRESHOLD(un + vn, 2 * MUL_TOOM4_THRESHOLD))
  {
    ...
    {
      l = (un + 4) / 5; // ceil(un/5)
      if ((((vn > 9 * k / 4) && (un + vn <= 6 * MUL_TOOM4_THRESHOLD)) || ((vn > 2 * l) && (un + vn > 6 * MUL_TOOM4_THRESHOLD))) && (vn <= 3 * l))
      {
